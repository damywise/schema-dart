  ...
  group('table_reader.dart', () {
    group('TableReader.fromConnectionString', () {
      ...
      test('- invalid string (password contains @ symbol)', () {
        expect(
            () => TablesReader.fromConnectionString(_invalidConnectionString),
...
class TableReaderMock extends TablesReader {
  TableReaderMock(String connectionString)
      : super.fromConnectionString(connectionString);
  ...
  String buildColumnTypesQuery(
      {required String schemaName, required List<String> tableNames}) {
    // intercept the filtered tables.
    _includedTables.addAll(tableNames);
    return super
        .buildColumnTypesQuery(schemaName: schemaName, tableNames: tableNames);
  }
}
